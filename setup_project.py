#!/usr/bin/env python3
"""
Quick setup script untuk membuat struktur folder dan file dasar
...
import streamlit as st
import pandas as pd
from utils.visualizations import create_prediction_chart, create_cost_breakdown_chart
...
def render_ai_chat(agentic_ai, rag_system):
    ...
    for i, question in enumerate(example_questions):
        with question_cols[i % len(question_cols)]:
            if st.button(f"❓ {question[:30]}...", key=f"q_{i}"):
                st.session_state['user_question'] = question
    
    # User input
    st.subheader("✍️ Ajukan Pertanyaan Anda")
...
def create_cost_breakdown_chart(cost_data: Dict[str, float]) -> go.Figure:
    """Buat chart breakdown biaya"""
    labels = [k.replace('_', ' ').title() for k in cost_data.keys()]
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        values=values,
        hole=.3,
        hovertemplate='<b>%{label}</b><br>Biaya: Rp %{value:,.0f}<br>Persentase: %{percent}<extra></extra>'
    )])
    
    fig.update_layout(
        title="Breakdown Komponen Biaya Haji",
        template='plotly_white',
        height=500
    )
...
'''
    ...
    # .env.example
    env_example_content = '''# API Keys - Copy to .env and fill with your actual keys
...
'''
    ...
    # README.md
    readme_content = '''# 🕌 RAG Agentic AI - Prediksi Biaya Haji Indonesia

Aplikasi prediksi biaya haji menggunakan RAG (Retrieval Augmented Generation) dan Agentic AI dengan data real-time harga emas dan nilai tukar.
...
3. **Optional - Add API Keys**:
   ...
   - Fill with your API keys for enhanced features

## 🎯 Fitur

- ✅ Prediksi biaya haji multi-skenario
- ✅ Data real-time harga emas (Finnhub)
- ✅ Nilai tukar USD/IDR (Fixer.io)
- ✅ AI Analysis dengan Qwen3 (OpenRouter)
- ✅ Visualisasi interaktif
- ✅ RAG System dengan knowledge base

## 🔧 Teknologi

- **Frontend**: Streamlit
- **AI**: Qwen3 via OpenRouter
- **Data**: Finnhub, Fixer.io
...
'''
    ...
    print("✅ Configuration files berhasil dibuat!")
...
def main():
    ...
    print("\\n✨ Enjoy your RAG Agentic AI Hajj Predictor!")
